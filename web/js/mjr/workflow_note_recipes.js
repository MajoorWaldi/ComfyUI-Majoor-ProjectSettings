...
function inferKindFromUrl(url) {
    ...
    if (lower.includes("/text_encoders/") || lower.includes("text_encoder"))
        return "text_encoders";
    if (lower.includes("/diffusion_models/") || lower.includes("diffusion_model"))
        return "diffusion_models";
    if (lower.includes("/clip_vision/"))
        return "clip_vision";
    if (lower.includes("/loras/") || lower.includes("/lora/"))
        return "loras";
    if (lower.includes("/vae/"))
        return "vae";
    if (lower.includes("/controlnet/"))
        return "controlnet";
    if (lower.includes("/upscale_models/") || lower.includes("/upscale/"))
        return "upscale_models";
    if (lower.includes("/clip/"))
        return "clip";
    if (lower.includes("/unet/"))
        return "unet";
    if (lower.includes("/embeddings/") || lower.includes("/embedding/"))
        return "embeddings";
    if (lower.includes("/checkpoints/") || lower.includes("/checkpoint/"))
        return "checkpoints";
    return "unknown";
}
...
function collectGraphsFromNode(node) {
    const n = node;
    const subgraph = n?.subgraph;
    const subgraphInstance = n?.subgraph_instance;
...
//# sourceMappingURL=workflow_note_recipes.js.map