...
import { buildFingerprintCache, downloadModels, fetchJSON, getDownloadStatus, getFingerprintCacheStatus, getModels, listExistingNames, previewTemplate, resolveModelRecipes, saveModelRecipes, scanModelCandidates, searchRegistry, } from "./mjr/api.js";
import { psConfirm, psPrompt } from "./mjr/dialog.js";
import { isSaveLikeNode, patchSaveNodes, stampGraphProjectSignature, } from "./mjr/patch.js";
import { applyFixesToGraph, scanMissingModelsWithStats, } from "./mjr/model_fixer.js";
...
export function buildPanel(el, state, actions) {
    ...
    const buildWorkflowPreview = () => {
        ...
        const next = base && state.workflowLastBase === base
    ...
    const refreshExistingNames = async (force = false) => {
        ...
        try {
            ...
            if (requestToken !== existingNamesToken)
                return;
            if (!names.length) {
                existingNamesSelect.style.display = "none";
                return;
    ...
    refreshProjectsList = async (force = false) => {
        ...
        if (!force && now - lastProjectsRefreshAt < throttleMs)
            return;
    ...
    const runFixMissingModels = async () => {
        ...
        try {
            ...
            for (const entry of missing) {
                ...
                const exact = candidates.filter((c) => Number(c?.score || 0) === 100 && !c?.in_wrong_folder && c?.reason !== "wrong_folder");
                if (exact.length === 1) {
            ...
            if (fixed + fixedCount > 0) {
                toast("success", "Fix applied", `Updated ${fixed + fixedCount} widget${fixed + fixedCount !== 1 ? "s" : ""}.`);
            }
            else {
    ...
    const basename = (value) => normalizeKey(String(value || ""));
    ...
    const runDownloadMissingModels = async () => {
        ...
        try {
            ...
            const rawFinalSummary = finalStatus?.summary;
            const summary = rawFinalSummary || {};
            const downloaded = Number(summary.downloaded || 0);
            const errors = Number(summary.errors || 0);
            const skipped = Number(summary.skipped || 0);
            if (downloadState.state === "error") {
                toast("error", "Download finished", `Downloaded ${downloaded}, skipped ${skipped}, errors ${errors}`);
            }
            else {
                toast("success", "Download finished", `Downloaded ${downloaded}, skipped ${skipped}, errors ${errors}`);
            }
            try {
                loadModels();
            }
    ...
    const loadModels = async () => {
        try {
            ...
            const extraCats = Object.keys(categories).filter((c) => !primaryCats.includes(c));
            const addGroup = (cat) => {
    ...
    createApplyBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            if (autoPatchCb.checked) {
                const patched = await patchSaveNodes(app, resp.rel_dir, resp.filename_prefix, resp.media);
                if (patched > 0) {
                    toast("success", "Patch applied", `Updated ${patched} nodes.`);
                }
                else {
    ...
    patchNowBtn.addEventListener("click", async () => {
        ...
        const confirmed = await psConfirm({
            title: "Patch save paths?",
            message: `Update ${nodeCount} save node${nodeCount !== 1 ? "s" : ""} to use the project path?\n\n` +
