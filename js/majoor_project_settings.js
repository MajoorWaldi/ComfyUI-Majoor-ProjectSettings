...
import { detectModelFromGraph, getSerializedWorkflow, inferProjectFolderFromGraph, readGraphSignature, } from "./mjr/graph.js";
import { PATH_WIDGETS, alreadyProjectPathed, detectNodeMedia, isSaveLikeNode, loadConfig, patchSaveNodes, patchSingleNode, stampGraphProjectSignature, } from "./mjr/patch.js";
...
const getSettingBool = (id, fallback) => {
    const raw = app?.extensionManager?.setting?.get?.(id) ??
...
async function createAndActivateProject(state, projectName, updateCallbacks) {
    const { updateStatus, updateResolve, updateWorkflowBlock, resetForProjectChange, updatePreview, refreshProjectsList, } = updateCallbacks;
...
async function promptEmptyGraphProject(state, runToken) {
    if (!state || state.projectId)
        return;
    if (state._emptyGraphPromptToken === runToken)
        return;
    ...
    if (state.graphLoadToken !== runToken)
        return;
    if (confirmCreate) {
...
async function onGraphLoadedDetectProject(state, token) {
    ...
    if (!graphEmpty) {
        ...
        if (sig && (sig.project_id || sig.project_folder)) {
            ...
            guardToast("info", "Project detected", `Workflow signature: ${detected.project_folder || detected.project_id}`);
        }
        else {
    ...
    if (detected.mode === "signature" &&
        ...
        guardToast("warn", "Project mismatch", `Workflow: ${detected.project_folder || detected.project_id}`);
    }
    if (!graphEmpty) {
...
function registerWorkflowShortcut() {
    ...
    window.addEventListener("keydown", (event) => {
        ...
        if (key !== "s")
            return;
        const state = runtimeState;
        if (!state || state.autoHookSave === false)
            return;
        if (!state.projectId)
            return;
        if (event.cancelable)
...
const EXTENSION_SETTINGS = [
    {
        ...
        type: "number",
        defaultValue: DEFAULT_DOWNLOAD_MAX_GB,
        min: 1,
        max: 1024,
        step: 1,
    ...
    },
    {
        ...
        type: "boolean",
        defaultValue: DEFAULT_AUTO_FIX_AFTER_DOWNLOAD,
        category: SETTINGS_CATEGORY,
    },
    {
        ...
        type: "boolean",
        defaultValue: DEFAULT_AUTO_OPEN_PANEL,
        category: SETTINGS_CATEGORY,
    },
    {
        ...
        type: "boolean",
        defaultValue: DEFAULT_AUTO_SCAN_MISSING_ON_OPEN,
        category: SETTINGS_CATEGORY,
    },
    {
        ...
        type: "boolean",
        defaultValue: DEFAULT_AUTO_REMEMBER_NOTE_SOURCES,
        category: SETTINGS_CATEGORY,
    },
    {
        ...
        type: "boolean",
        defaultValue: DEFAULT_NO_PROJECT_STATUS_BLACK,
        category: SETTINGS_CATEGORY,
    },
];
function registerLegacyExtensionSettings() {
    ...
    try {
        ...
        for (const setting of EXTENSION_SETTINGS) {
            const fallback = setting.type === "number"
                ? toNumber(values[setting.id], setting.defaultValue)
                : toBool(values[setting.id], setting.defaultValue);
            settings.addSetting({ ...setting, defaultValue: fallback });
        }
    }
    catch (e) {
...
app.registerExtension({
    ...
    async afterConfigureGraph() {
        if (runtimeState) {
            ...
            if (getSettingBool(AUTO_SCAN_MISSING_ON_OPEN_SETTING, DEFAULT_AUTO_SCAN_MISSING_ON_OPEN)) {
                ...
                setTimeout(() => {
                    if (!runtimeState || runtimeState.graphLoadToken !== scanToken)
                        return;
                    try {
    ...
    async nodeCreated(node) {
        ...
        await loadConfig();
        if (!state.projectId || !state.autoPatchNewNodes)
            return;
        if (!state.projectFolder || state.projectExists === false)
            return;
        if (!isSaveLikeNode(node))
            return;
...
//# sourceMappingURL=majoor_project_settings.js.map