...
import { extractFilenameFromUrl, getAllowedExtensions, hasAllowedExtension, isValidUrl, } from "./model_downloader.js";
function formatRecipeText(recipe) {
    const parts = [];
    if (recipe?.url)
        parts.push(recipe.url);
    if (recipe?.filename)
        parts.push(`file: ${recipe.filename}`);
    if (recipe?.kind)
        parts.push(`kind: ${recipe.kind}`);
    if (recipe?.sha256)
        parts.push(`sha256: ${recipe.sha256}`);
    return parts.join(" | ");
}
function showSearchResultsDialog(results, callback) {
    ...
    for (const result of results) {
        ...
        const infoParts = [];
        if (result.type)
            infoParts.push(`Type: ${result.type}`);
        if (result.version)
            infoParts.push(`Ver: ${result.version}`);
        if (result.size_mb)
            infoParts.push(`Size: ${result.size_mb}MB`);
        if (result.filename)
            infoParts.push(`File: ${result.filename}`);
                    ...
                    showSearchResultsDialog(allResults, (selectedResult) => {
                        if (selectedResult) {
                            ...
                            if (selectedResult.filename)
                                fileField.value = selectedResult.filename;
                            if (selectedResult.sha256)
                                shaField.value = selectedResult.sha256;
                            if (selectedResult.type)
                                setKindValue(selectedResult.type);
...
//# sourceMappingURL=ui_model_downloader_dialog.js.map